...
//! into motion resolution (`vimci-motion`) and command execution
//! (`vimci-cmd`), against a live [`Session`].
//!
...
use vimci_core::{Frame, Register, Timeline, TrackId};
use vimci_motion::{
...
impl Engine {
    ...
    fn jump_points(&self, tl: &Timeline) -> vimci_motion::JumpPoints {
        vimci_motion::JumpPoints::build(
            tl,
            ...
            Ok(Resolved::Position(p)) => {
                if self.mode.mode().is_visual() {
                    self.mode.extend_visual(
                        ...
                        vimci_motion::Direction::Forward,
                    );
    ...
    fn do_yank(
        ...
        let reg = session
            ...
            .map_err(vimci_cmd::CmdError::from)?;
...
fn run(result: Result<String, vimci_cmd::CmdError>) -> Outcome {
    match result {
...
fn clip_under(tl: &Timeline, track: TrackId, frame: Frame) -> Option<vimci_core::ClipId> {
...
fn nearest_edge(
    ...
    track: TrackId,
    frame: Frame,
) -> Option<(vimci_core::ClipId, vimci_core::Edge)> {
    ...
    Some((
        c.id,
        if to_head <= to_tail {
            vimci_core::Edge::Head
        } else {
            vimci_core::Edge::Tail
        },
    ))
}
...
fn edge_frame(
    tl: &Timeline,
    track: TrackId,
    clip: vimci_core::ClipId,
    edge: vimci_core::Edge,
) -> Option<Frame> {
    let t = tl.track(track)?;
    let c = t.clip(clip)?;
    Some(match edge {
        vimci_core::Edge::Head => c.start,
        vimci_core::Edge::Tail => c.end(),
    })
}

...
//!   [`vimci_core::Timeline`], which is what makes golden key-string tests
//!   possible with no fixture.
//! - [`engine`] - gives the parsed [`action::Action`] meaning against a live
//!   [`vimci_cmd::Session`], using [`vimci_motion`] to resolve targets and
//!   [`vimci_cmd`] to apply them. Transport is dispatched separately from

...
use vimci_analysis::analysis::{AnalysisParams, analyze_samples};
use vimci_analysis::cache::AnalysisCache;
use vimci_analysis::conform::{ConformOptions, conform};
use vimci_analysis::import::{ImportOptions, import};
use vimci_analysis::probe::{FfprobeProber, Prober, StreamKind};
use vimci_analysis::proxy::{ProxyPolicy, plan_proxy};
use vimci_analysis::{decode, subtitle};
use vimci_cmd::Session;
use vimci_core::{Fps, Frame, Timeline, TimelineProps};
...
fn probe(name: &str) -> vimci_analysis::MediaInfo {
    FfprobeProber
...
fn a_proxy_has_exactly_the_same_frame_count_as_its_source() {
    ...
    let dir = std::env::temp_dir().join(format!("vimci-proxy-{}", std::process::id()));
    ...
    vimci_analysis::proxy::generate(&spec, None).unwrap();
...
fn a_cached_analysis_survives_a_round_trip_through_disk() {
    let dir = std::env::temp_dir().join(format!("vimci-cache-slow-{}", std::process::id()));
    let cache = AnalysisCache::for_project(&dir);
    let source = fixtures().join("tone_gaps.wav");
    let hash = vimci_analysis::content_hash(&source).unwrap();

...
//! `.vimci/cache/<content_hash>.analysis` next to the project and reused on
...
/// A project's `.vimci/cache` directory.
...
impl AnalysisCache {
    ...
    pub fn for_project(project_dir: &Path) -> Self {
        Self {
            root: project_dir.join(".vimci").join("cache"),
        }
    }
...
mod tests {
    ...
    fn tmpdir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("vimci-cache-{name}-{}", std::process::id()));

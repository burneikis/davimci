...
mod tests {
    ...
    fn tmpdir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("vimci-pipe-{name}-{}", std::process::id()));
    ...
    fn analysing_a_missing_file_is_offline_media_not_a_crash() {
        ...
        .unwrap_err();
        assert_eq!(
            vimci_core::Classify::class(&err),
            vimci_core::ErrorClass::OfflineMedia
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn a_queued_analysis_publishes_and_only_then_answers() {
        ...
        let query =
            |idx: &AnalysisIndex| idx.find(&predicate, vimci_core::Frame::ZERO, Direction::Forward);

...
use vimci_core::{Fps, Frame, Timeline};

use crate::conform::Conformed;
use crate::error::AnalysisError;
use crate::probe::{MediaInfo, StreamInfo};

/// Proxy settings, mirroring `vimci.media.configure` in spec §10.3.
...
mod tests {
    ...
    use vimci_core::{Clip, MediaRef, Resolution, TimelineProps};
    ...
    fn export_is_refused_while_a_clip_resolves_to_a_proxy() {
        ...
        proxies.insert("/p/.vimci/cache/abc.proxy.mov", "/media/original.mkv");

        let ok = timeline_using("/media/original.mkv");
        assert!(export_guard(&ok, &proxies).is_ok());

        let bad = timeline_using("/p/.vimci/cache/abc.proxy.mov");
        ...
        assert!(vimci_core::Classify::user_message(&err).contains("original"));
    }

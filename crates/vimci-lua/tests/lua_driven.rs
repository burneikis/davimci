...
//! motion and keymaps, the harness feeds keys through `vimci-keys`, and the
...
use vimci_keys::{Engine, Key, Keymap, Outcome};
use vimci_lua::{MotionAnswer, MotionEnv, Opts, Request, Runtime, Sample, Sandbox, TrackData};
...
fn a_lua_motion_bound_to_a_key_resolves_against_the_analysis_snapshot() {
    ...
    opts.insert("track", vimci_lua::OptValue::Str("A1".into()));
    opts.insert("threshold_db", vimci_lua::OptValue::Num(-2.0));
    assert_eq!(

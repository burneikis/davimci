...
use vimci_core::{Classify, ErrorClass, Notice};
...
pub enum Sandbox {
    /// The user's own `~/.config/vimci` tree: full standard library.
    Trusted,
    /// Project-local `.vimci.lua`: no `os`, no `io`, no `load`/`dofile`, and
    /// a `require` that resolves nothing but the `vimci.*` modules.
    Restricted,
}
...
impl Runtime {
    /// A runtime with the `vimci.*` modules installed and nothing loaded.
    ...
    fn restricted_env(&self) -> mlua::Result<Table> {
        ...
        for name in [
            ...
            "tostring", "type", "unpack", "xpcall", "math", "string", "table", "vimci",
        ] {
        ...
        // A `require` that resolves the vimci modules and nothing else, so
        // the documented `require("vimci.keymap")` form still works.
        let loaded: Table = g.get::<Table>("package")?.get("loaded")?;
        let require = self.lua.create_function(move |_, name: String| {
            if !name.starts_with("vimci") {
    ...
    /// User bindings in the shape `vimci_keys::Keymap` layers over the
    /// defaults. A Lua function becomes [`Action::Plugin`], which the host
    /// hands back to [`Runtime::invoke`].
    ///
    /// Only `NORMAL` bindings are returned today: `vimci_keys::Keymap` is one

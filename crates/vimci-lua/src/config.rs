//! `vimci.timeline.configure` (spec §9.6): the jump-point and frame-step
//! settings a config may change.
//!
//! The output is a [`JumpConfig`] - the same type `vimci-motion` already
//! consumes - so a configured editor and a default one differ by data, never
//! by code path.

use vimci_keys::Key;
use vimci_motion::{JumpConfig, JumpSources};

use crate::error::LuaError;

/// Everything `vimci.timeline.configure` can set.

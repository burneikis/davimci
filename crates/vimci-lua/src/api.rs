//! The `vimci.*` module surface (spec §9.2 - §9.8).
//!
//! Every module is a plain table published into `package.loaded`, so the
//! documented `require("vimci.keymap")` form works without a `package.path`
...
pub(crate) fn install(lua: &Lua, state: &Shared) -> mlua::Result<()> {
    let vimci = lua.create_table()?;
    vimci.set("version", env!("CARGO_PKG_VERSION"))?;
    ...
    for (name, table) in modules {
        loaded.set(format!("vimci.{name}"), table.clone())?;
        vimci.set(name, table)?;
    }
    loaded.set("vimci.editor", editor.clone())?;
    vimci.set("editor", editor)?;
    loaded.set("vimci", vimci.clone())?;
    lua.globals().set("vimci", vimci)?;
    Ok(())
}
    ...
    t.set(
        ...
        lua.create_function(move |_, (event, f): (String, Function)| {
            if !EVENTS.contains(&event.as_str()) {
                return Err(err(LuaError::Config(format!(
                    "'{event}' is not a vimci event (known: {})",
                    EVENTS.join(", ")
                ))));
            }
            let mut st = st.borrow_mut();

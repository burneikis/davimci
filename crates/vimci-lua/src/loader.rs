...
impl ConfigPaths {
    ...
    /// `$XDG_CONFIG_HOME/vimci`, falling back to `~/.config/vimci`.
    #[must_use]
    pub fn from_env() -> Option<Self> {
        let base = std::env::var_os("XDG_CONFIG_HOME")
            .map(PathBuf::from)
            .or_else(|| std::env::var_os("HOME").map(|h| PathBuf::from(h).join(".config")))?;
        Some(Self::new(base.join("vimci")))
    }
...
impl Runtime {
    ...
    /// Load `<dir>/.vimci.lua` if the user trusts it (spec §9.7).
    ///
...
mod tests {
    ...
    fn a_missing_config_tree_is_not_an_error() {
        let paths = ConfigPaths::new("/nonexistent/vimci-config");

...
use vimci_backend::RenderSettings;
use vimci_core::{Fps, Resolution};
...
/// Containers vimci will mux, and the codecs each accepts. A pairing outside
    ...
    pub fn validate(&self) -> Result<(), LuaError> {
        let Some((_, video, audio)) = CONTAINERS
            ...
            return Err(LuaError::Config(format!(
                "export preset '{}' asks for container '{}', which vimci cannot write (known: {})",
                self.name,
                self.container,
                known.join(", ")
            )));

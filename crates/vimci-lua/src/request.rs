...
//! `vimci.editor` does not edit: it appends a [`Request`], and the host
//! (a frontend, or the headless test harness) turns each one into a
//! `vimci_keys::Action` and runs it against the session. That keeps the undo
...
pub enum Request {
    ...
    Edit(Action),
    /// `require("vimci.export").run(name)`.
    Export { preset: String },
    /// `require("vimci.motions").run(name, opts)` - resolved by the host,
    /// which owns the timeline and the analysis index.
    Motion { name: String, opts: Opts },
    /// `require("vimci.media").import(path)`.
    Import { path: String },
    /// `require("vimci.media").analyze(track)` - re-run analysis after a
    /// gain or fade change (plan.md Phase 9e).
    Analyze { track: Option<String> },
    /// `require("vimci.editor").message(text)`, for the status line.
    Message(String),
}

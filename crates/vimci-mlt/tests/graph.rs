...
use vimci_backend::{PreviewScale, RenderBackend};
use vimci_core::testing::{clip_ids, fixture, track_id};
use vimci_core::{Frame, Resolution, TimelineProps, TrackKind};
use vimci_mlt::MltBackend;
...
fn offline_media_renders_as_a_placeholder_rather_than_failing() {
    let mut b = backend();
    let mut tl = vimci_core::testing::media_fixture(&[(0, 50, 0, 500)]);
    tl.props = props();
...
fn probing_missing_media_reports_it_offline() {
    ...
    let err = b
        ...
        .unwrap_err();
    assert!(matches!(err, vimci_backend::BackendError::Offline { .. }));
}

...
use vimci_core::{Clip, ClipId, Fps, Frame, MediaRef, Resolution, Timeline, TimelineProps};
...
fn a_render_produces_a_file_with_the_requested_streams() {
    ...
    let out = std::env::temp_dir().join("vimci-render-smoke.mkv");
...
fn a_cancelled_render_stops_reporting_progress() {
    ...
    let out = std::env::temp_dir().join("vimci-render-cancel.mkv");
    let _ = std::fs::remove_file(&out);

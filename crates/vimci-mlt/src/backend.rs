...
//! consumer as RGBA and handed to `vimci-present`. MLT never opens a window
...
use vimci_core::{Fps, Frame, Resolution, Timeline, TimelineProps};
use vimci_mlt_sys as sys;
...
impl MltBackend {
    ...
    fn build_producer(&self, entry: &ClipEntry) -> Result<Producer> {
        ...
        };
        {
            ...
            props
                .set("vimci.clip", &entry.clip.to_string())
                .map_err(BackendError::from)?;
            ...
            if let Resource::Offline { path } = &entry.resource {
                props
                    .set("vimci.offline", path)
                    .map_err(BackendError::from)?;
            }
        }
        // Normalisers. A consumer would plant these; vimci pulls frames

...
fn a_keymap_naming_an_unknown_command_is_rejected_when_the_config_loads() {
    ...
    let e = rt
        .exec(
            r#"require("davimci.keymap").map("normal", "s", "editor.frobnicate")"#,
            "bad.lua",
            Sandbox::Trusted,
        )
...
fn an_object_with_neither_form_is_rejected() {
    ...
    assert!(
        rt.exec(
            r#"require("davimci.textobject").register("z", {})"#,
            "z.lua",
            Sandbox::Trusted
        )
        .is_err()
...
fn a_named_track_list_is_accepted_the_way_the_comment_says() {
    ...
    exec(
        &rt,
        r#"
        require("davimci.export").preset("p", {
...
fn an_impossible_codec_pairing_is_refused_at_definition() {
    ...
    let e = rt
        .exec(
            r#"require("davimci.export").preset("bad", { container = "webm", video_codec = "h264" })"#,
            "bad.lua",
            Sandbox::Trusted,
        )
        .expect_err("must reject");
...
fn a_rejected_configure_leaves_the_previous_settings_intact() {
    ...
    assert!(
        rt.exec(
            r#"require("davimci.timeline").configure({ jump_points = { "clip_bounds", "beats" } })"#,
            "bad.lua",
            Sandbox::Trusted,
        )
        .is_err()
...
fn spec_9_8_handlers_load_and_fire_for_their_own_event_only() {
    ...
    exec(
        ...
        r#"
        seen = {}
        require("davimci.autocmd").on("PlayheadMoved", function(e)
          seen[#seen + 1] = e.frame
        end)
        "#,
    );
...
fn an_unknown_event_name_is_rejected_at_registration() {
    ...
    let e = rt
        .exec(
            r#"require("davimci.autocmd").on("SplitPerfomed", function() end)"#,
            "typo.lua",
            Sandbox::Trusted,
        )
...
fn handlers_run_in_registration_order_and_a_veto_stops_the_rest() {
    ...
    exec(
        ...
        r#"
        order = {}
        local au = require("davimci.autocmd")
...
fn a_non_cancellable_event_ignores_a_false_return() {
    ...
    exec(
        &rt,
        r#"require("davimci.autocmd").on("ProjectLoaded", function() return false end)"#,
    );
...
fn a_handler_can_be_removed_with_off() {
    ...
    exec(
        ...
        r#"
        count = 0
        id = require("davimci.autocmd").on("PlayheadMoved", function() count = count + 1 end)
        require("davimci.autocmd").off(id)
        "#,
    );
...
fn a_throwing_keymap_callback_is_disabled_and_queues_nothing() {
    ...
    exec(
        &rt,
        r#"
        require("davimci.keymap").map("normal", "Z", function()
          require("davimci.editor").ripple_delete()
          error("plugin bug")
        end)
        "#,
    );
...
fn a_broken_config_file_costs_only_that_file() {
    ...
    std::fs::write(
        root.join("init.lua"),
        r#"require("davimci.keymap").map("normal", "s", "editor.split_at_playhead")"#,
    )
    .unwrap();
    ...
    std::fs::write(
        ...
        r#"require("davimci.keymap").map("normal", "x", "editor.ripple_delete")"#,
    )
    .unwrap();
...
fn an_untrusted_project_local_config_does_not_run() {
    ...
    std::fs::write(
        dir.join(".davimci.lua"),
        r#"require("davimci.keymap").map("normal", "Q", "editor.undo")"#,
    )
    .unwrap();
    let rt = rt();
...
fn a_trusted_project_local_config_runs_but_cannot_reach_os_or_io() {
    ...
    std::fs::write(
        dir.join(".davimci.lua"),
        r#"
        require("davimci.export").preset("local", { container = "mkv", video_codec = "h264" })
        reached_os = (os ~= nil)
        reached_io = (io ~= nil)
        "#,
    )
...
fn a_sandboxed_chunk_has_no_os_execute_no_io_and_no_arbitrary_require() {
    let rt = rt();
    for src in [
        "os.execute('touch /tmp/davimci-should-not-exist')",
        "io.open('/etc/passwd')",
        "dofile('/etc/passwd')",
        "load('return 1')()",
        "require('os')",
...
fn editor_calls_queue_requests_instead_of_editing() {
    ...
    exec(
        ...
        ed.step_frame(-3)
        ed.message("hello")
        require("davimci.media").import("/tmp/a.mkv")
        "#,
    );
    assert_eq!(
        rt.take_requests(),
...
fn an_autocmd_may_ask_for_an_edit_and_the_host_gets_it_back() {
    ...
    exec(
        &rt,
        r#"
        require("davimci.autocmd").on("SplitPerformed", function(e)
          require("davimci.editor").message("split at " .. e.frame)
        end)
        "#,
    );
